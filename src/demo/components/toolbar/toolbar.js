export default {
    computed: {
        ...
        angle(){
            return this.activeRect === null ? '' : this.$store.state.rect.rects[this.activeRect].angle
        },
    ...
    methods: {
        ...
        changeAngle(ev){
            let angle = parseInt(ev.target.value);
            if (typeof angle !== 'number' || isNaN(angle)) {
                angle = 0;
            }
            this.$store.dispatch('rect/setAngle', {id: this.activeRect, angle: angle});
        },
